...
export class StreamInput {
    ...
    setPeer(peer: RTCPeerConnection) {
        ...
        this.keyboard = this.createDataChannelWithReconnect(peer, "keyboard")

        this.mouseClicks = this.createDataChannelWithReconnect(peer, "mouseClicks", {
            ordered: false
        })
        this.mouseAbsolute = this.createDataChannelWithReconnect(peer, "mouseAbsolute", {
            ordered: true,
            maxRetransmits: 0
        })
        this.mouseRelative = this.createDataChannelWithReconnect(peer, "mouseRelative", {
            ordered: false
        })

        this.touch = this.createDataChannelWithReconnect(peer, "touch")
        this.touch.onmessage = this.onTouchMessage.bind(this)

        this.controllers = this.createDataChannelWithReconnect(peer, "controllers")
        this.controllers.addEventListener("message", this.onControllerMessage.bind(this))
    }

    private createDataChannelWithReconnect(peer: RTCPeerConnection, label: string, options?: RTCDataChannelInit): RTCDataChannel {
        const channel = peer.createDataChannel(label, options);
        this.setupChannelReconnection(channel, peer, label, options);
        return channel;
    }

    private setupChannelReconnection(channel: RTCDataChannel, peer: RTCPeerConnection, label: string, options?: RTCDataChannelInit) {
        channel.addEventListener("close", () => {
            if (peer.connectionState === "connected" || peer.connectionState === "connecting") {
                console.warn(`[Input] Data channel '${label}' closed unexpectedly. Attempting to reconnect...`);
                setTimeout(() => {
                    if (peer.connectionState === "connected" || peer.connectionState === "connecting") {
                        const newChannel = peer.createDataChannel(label, options);
                        this.setupChannelReconnection(newChannel, peer, label, options);

                        // Re-assign to class properties based on label
                        if (label === "keyboard") this.keyboard = newChannel;
                        else if (label === "mouseClicks") this.mouseClicks = newChannel;
                        else if (label === "mouseAbsolute") this.mouseAbsolute = newChannel;
                        else if (label === "mouseRelative") this.mouseRelative = newChannel;
                        else if (label === "touch") {
                            this.touch = newChannel;
                            this.touch.onmessage = this.onTouchMessage.bind(this);
                        }
                        else if (label === "controllers") {
                            this.controllers = newChannel;
                            this.controllers.addEventListener("message", this.onControllerMessage.bind(this));
                        }

                        console.info(`[Input] Reconnected data channel '${label}'`);
                    }
                }, 500);
            }
        });
    ...
    onStreamStart(capabilities: StreamCapabilities, streamerSize: [number, number]) {
        ...
        // Re-announce virtual gamepads
        for (let i = 0; i < this.gamepads.length; i++) {
            if (this.gamepads[i] === -2) {
                this.sendControllerAdd(i, SUPPORTED_BUTTONS, StreamControllerCapabilities.CAPABILITY_RUMBLE)
            }
        }
    }

    // -- Keyboard
    private shouldBlockKey(event: KeyboardEvent): boolean {
        ...
        // Block Alt+F4
        if (event.altKey && (event.key === "F4" || event.code === "F4")) {
            return true;
        }

        // Block Ctrl+Shift+Esc
        if (event.ctrlKey && event.shiftKey && (event.key === "Escape" || event.code === "Escape")) {
            return true;
        }
    ...
    public registerVirtualGamepad(): number {
        const id = this.gamepads.length
        this.gamepads.push(-2) // Use -2 as marker for virtual

        // Send add event if connected
        if (this.connected) {
            this.sendControllerAdd(id, SUPPORTED_BUTTONS, StreamControllerCapabilities.CAPABILITY_RUMBLE)
        }
        return id
    }

    public sendVirtualGamepadState(id: number, state: GamepadState) {
    ...
    private onControllerMessage(event: MessageEvent) {
        ...
        if (ty == 0) {
            ...
            if (gamepadIndex == null || gamepadIndex === -2) {
                return
            }

    ...
    private onIceConnectionStateChange() {
        ...
        if (state === "disconnected" || state === "checking") {
            const customEvent: InfoEvent = new CustomEvent("stream-info", {
                detail: {
                    type: "addDebugLine",
                    line: "⚠️ Network Instability Detected"
                }
            })
            this.eventTarget.dispatchEvent(customEvent);

            // Optionally, we could show a visible error if 'error' support is generic enough
            const errorEvent: InfoEvent = new CustomEvent("stream-info", {
                detail: { type: "stageFailed", stage: "Network Stability", errorCode: -999 }
            })
            // We use stageFailed with a fake error code just to flash a message if needed, 
            // or we can rely on addDebugLine if the user has logs open. 
